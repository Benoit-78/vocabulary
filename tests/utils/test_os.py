...
from src.utils import os as utils_os
...
class TestUtils(unittest.TestCase):
    ...
    def test_get_os_type_error(self, mock_platform):
        """Error should be raised in case of unknown utils_os."""
        # Arrange
        mock_logger = MagicMock()
        logging.basicConfig(level=logging.ERROR)
        with patch('src.utils.os.logger', mock_logger):
            # Act
            _ = utils_os.get_os_type()
        # Assert
    ...
    @patch('src.utils.os.get_os_type', return_value='Windows')
    def test_get_os_separator_windows(self, mock_get_os_type):
        # Act
        result = utils_os.get_os_separator()
        # Assert
        self.assertEqual(result, '\\')

    @patch('src.utils.os.get_os_type', return_value='Linux')
    ...
    @patch('src.utils.os.get_os_type', return_value='Mac')
    ...
    @patch('src.utils.os.get_os_type', return_value='Android')
    ...
    @patch('src.utils.os.get_os_type', return_value='Unknown')
    def test_get_os_separator_unknown(self, mock_get_os_type):
        with self.assertRaises(NameError):
            # Assert
            utils_os.get_os_separator()
